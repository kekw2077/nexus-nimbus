...
class NextcloudException implements Exception {
  ...
  factory NextcloudException.fromStatus(int code, Uri uri) {
    final text = switch (code) {
      ...
      // Яндекс отдаёт 402, когда WebDAV закрыт тарифом: доступ по этому
      // протоколу у них оставлен платным подпискам.
      402 => 'Сервер требует оплаченную подписку для этого доступа. '
          'У Яндекс.Диска WebDAV работает только с платным Яндекс 360 — '
          'логин и пароль тут ни при чём.',
